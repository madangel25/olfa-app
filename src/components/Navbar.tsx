...
export function Navbar({ compact = false, className = "" }: NavbarProps) {
  ...
  return (
    <nav
      className={`${navModeClass} z-50 ${heightClass} border-b border-stone-200 shadow-[0_1px_3px_rgba(0,0,0,.07)] font-[family-name:var(--font-jakarta)] ${className}`}
      role="navigation"
      ...
      <div className={navInnerClass}>
        ...
        >
          ...
          <div>
            <Link
              href="/"
              className={`flex items-center gap-2 text-lg font-semibold text-stone-900 transition ${accentClass} hover:opacity-90`}
            ...
            {isLoggedIn && (
              <div className="relative">
                <button
                  ...
                  className={`flex items-center gap-2 rounded-xl border ${borderClass} bg-white px-3 py-2 text-sm font-medium text-stone-700 shadow-sm transition ${hoverBgClass} focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-[var(--theme-primary)] focus:opacity-80`}
                ...
                {dropdownOpen && (
                  <>
                    ...
                    <div
                      className={`absolute top-full z-50 mt-2 w-72 max-w-[calc(100vw-1rem)] overflow-hidden rounded-2xl border border-stone-200 bg-white p-2 shadow-xl ${locale === "ar" ? "left-0" : "right-0"}`}
                      role="menu"
                      aria-orientation="vertical"
                      aria-labelledby="user-menu-button"
                    >
                      <div className={`rounded-xl border ${borderClass} bg-stone-50 px-4 py-3`}>
                      ...
                      <Link
                        ...
                        className={`mt-2 flex items-center gap-3 rounded-xl px-4 py-3 text-sm text-stone-700 transition ${hoverBgClass} ${
                          isRtl ? "flex-row-reverse text-right" : "text-left"
                        }`}
                        role="menuitem"
                        onClick={() => setDropdownOpen(false)}
                      ...
                      <Link
                        href="/settings"
                        className={`mt-1 flex items-center gap-3 rounded-xl px-4 py-3 text-sm text-stone-700 transition ${hoverBgClass} ${
                          isRtl ? "flex-row-reverse text-right" : "text-left"
                        }`}
                        role="menuitem"
                        onClick={() => setDropdownOpen(false)}
                      ...
                      <button
                        ...
                        className={`mt-2 flex w-full items-center gap-3 rounded-xl px-4 py-3 text-sm text-stone-700 transition hover:bg-stone-50 hover:text-stone-900 ${
                          isRtl ? "flex-row-reverse text-right" : "text-left"
                        }`}
                        role="menuitem"
                      >
            ...
            <div className="relative">
              ...
              {langDropdownOpen && (
                <>
                  ...
                  <div
                    className={`absolute top-full z-50 mt-2 min-w-[10rem] overflow-hidden rounded-xl border border-stone-200 bg-white py-1 shadow-xl ${locale === "ar" ? "left-0" : "right-0"}`}
                    role="menu"
                    aria-label={locale === "ar" ? "اللغة" : "Language"}
                  >
                    <button
            ...
            {!isLoggedIn &&
              ...
              pathname !== "/register" && (
                <>
                  <Link
                    href="/login"
                    className={`rounded-lg px-2 py-1 text-sm font-medium transition ${pathname === "/login" ? accentClass : `text-stone-600 ${hoverBgClass} hover:opacity-90`}`}
                  >
                    {t("nav.login")}
                  </Link>
                  <Link
                    href="/register"
                    className={`rounded-lg px-2 py-1 text-sm font-medium transition ${pathname === "/register" ? accentClass : `text-stone-600 ${hoverBgClass} hover:opacity-90`}`}
                  >
                    {t("nav.register")}
                  </Link>
                </>
